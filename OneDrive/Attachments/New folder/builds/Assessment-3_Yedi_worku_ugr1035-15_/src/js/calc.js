...
// Function to calculate the result following standard PEMDAS precedence
function calculateResult() {
    if (currentInput.trim() === "") return; // No input, do nothing

    try {
        // Parse the input into tokens (numbers and operators)
        let tokens = currentInput.match(/(\d+(\.\d+)?|[+\-*/])/g); // Split input into numbers and operators
        if (!tokens) throw new Error("Invalid Expression");

        // Perform operations in the correct order: Division, Multiplication, Addition, Subtraction
        tokens = performOperation(tokens, "/"); // Handle division first
        ...
        // The final result is the only token left
...
function performOperation(tokens, operator) {
    while (tokens.includes(operator)) {
        ...
        if (operator === "/" && right === 0) throw new Error("Division by zero"); // Handle division by zero
